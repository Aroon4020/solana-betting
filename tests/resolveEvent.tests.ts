...
describe("event-betting extended tests: resolve_event, claim_winnings, withdraw_fees", () => {
    ...
    const feePercentage = new anchor.BN(200); // 2%
    ...
    before(async () => {
        ...
        async function createATAIfNotExist(ataPublicKey, mint, ownerPublicKey, payerKeypair) {
            try {
                await getAccount(connection, ataPublicKey);
            } catch (error) {
        ...
        const nextEventId = (await program.account.programState.fetch(programStatePDA)).nextEventId;
        const [_eventPDA, _eventBump] = await PublicKey.findProgramAddress(
        ...
        const programStateAfterCreate = await program.account.programState.fetch(programStatePDA);
        const actualEventId = programStateAfterCreate.nextEventId - 1; // Since nextEventId was incremented
